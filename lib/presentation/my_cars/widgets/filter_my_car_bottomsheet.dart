...
Future<dynamic> filterMyCarBottomSheet(Map<String, dynamic> filterData) {
  return showModalBottomSheet(
    ...
    builder: (context) => MyCarsFilterBottomSheetWidget(filterData: filterData),
  );
}

class MyCarsFilterBottomSheetWidget extends StatefulWidget {
  final Map<String, dynamic> filterData;
  const MyCarsFilterBottomSheetWidget({
    Key? key,
    required this.filterData,
  }) : super(key: key);

  @override
  State<MyCarsFilterBottomSheetWidget> createState() =>
      _MyCarsFilterBottomSheetWidgetState();
}

class _MyCarsFilterBottomSheetWidgetState
    extends State<MyCarsFilterBottomSheetWidget> {
  ...
  List<CarStatus> selectedCarStatus = [];
  ...
  void setStatusTypes() {
    carStatusList = CarStatus.values.map((e) => e).toList();
    carStatusList.removeWhere((element) => element == CarStatus.disabled);
  }
  ...
  Widget statusFilter() {
    return FilterExpansionTile(
      ...
      title: userTypeLabel.toUpperCase(),
      subTitle: youCanSelectMultipleStatus,
      children: ListView.separated(
        shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
        ...
        itemBuilder: (_, int index) {
          ...
          return CustomCheckboxWithLabel(
            label: carStatusToText(status),
            value: isSelectedStatus(selectedCarStatus, status),
            onChanged: (value) {
              if (isSelectedStatus(selectedCarStatus, status)) {
                if (carStatusToText(status) == 'UNLISTED') {
                  for (var element in unListCarStatus) {
                    selectedCarStatus
                        .remove(getSelectedStatus(selectedCarStatus, element));
                  }
                } else {
                  selectedCarStatus
                      .remove(getSelectedStatus(selectedCarStatus, status));
                }
              } else {
                if (carStatusToText(status) == 'UNLISTED') {
                  for (var element in unListCarStatus) {
                    selectedCarStatus.add(element);
                  }
                } else {
                  selectedCarStatus.add(status);
                }
              }
            },
          );
  ...
  String carStatusToText(CarStatus item) {
    switch (item) {
  ...
  bool isSelectedStatus(List<CarStatus> list, CarStatus item) {
    return list.any((element) => element == item);
  }

  CarStatus getSelectedStatus(List<CarStatus> list, CarStatus item) {
    return list.where((element) => element == item).first;
  }
  ...
  Future<void> onTapApply() async {
    final Map<String, dynamic> selectedFilters = {
      ...
      if (selectedCarStatus.isNotEmpty) 'status': selectedCarStatus,
    };
  ...
  void populateData(Map<String, dynamic> filterJson) {
    ...
    if (filterJson.containsKey(key.statusType)) {
      selectedCarStatus = List.from(filterJson[key.statusType] as List);
    }
    setState(() {});
  }
}
